...
def _rules(**safety_overrides) -> RulesConfig:
    ...
    return RulesConfig(
        ...
        hard_stop_conditions=[
            CAPTCHA_DETECTED, MFA_DETECTED, CONSENT_REQUIRED, UNEXPECTED_FORM_STRUCTURE,
        ],
...
def test_real_config_rules_yaml_has_stop_flags_enabled_by_default():
    ...
    assert cfg.rules.safety.stop_on_consent_required is True
...
def test_consent_required_requires_human_when_rule_enabled():
    rules = _rules(stop_on_consent_required=True)
    inspection = ApplicationInspection(structure_recognized=True, consent_required=True)
    verdict = evaluate_inspection(rules, inspection)
    assert verdict.human_required is True
    assert verdict.reason == CONSENT_REQUIRED


def test_consent_required_ignored_when_the_actual_rule_is_disabled():
    rules = _rules(stop_on_consent_required=False)
    inspection = ApplicationInspection(structure_recognized=True, consent_required=True)
    verdict = evaluate_inspection(rules, inspection)
    assert verdict.human_required is False


def test_consent_required_takes_priority_over_unrecognized_structure():
    rules = _rules()
    inspection = ApplicationInspection(structure_recognized=False, consent_required=True)
    verdict = evaluate_inspection(rules, inspection)
    assert verdict.reason == CONSENT_REQUIRED


def test_mfa_takes_priority_over_consent_required():
    rules = _rules()
    inspection = ApplicationInspection(
        structure_recognized=False, mfa_detected=True, consent_required=True,
    )
    verdict = evaluate_inspection(rules, inspection)
    assert verdict.reason == MFA_DETECTED
